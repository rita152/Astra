...
| `item/completed` | 服务端通知 | 默认 | 要求对象 `item` 及字符串 `type/id`；当前严格承接文本 `userMessage`、`agentMessage`、`commandExecution`、`reasoning` 与 `fileChange`；fileChange 严格读取 `status` 及每项 `path/diff/kind`，包括 update 的 nullable `move_path`；commandExecution 严格读取并保留 `commandActions` 的 `read`／`listFiles`／`search`／`unknown` 变体，reasoning 另要求整数 `completedAtMs` 及字符串数组 `summary/content` | 单 reader → `ManagedTurn`（response 前按 wire order 缓存）→ `process_turn_message`、各 item parser | 只向 owner turn 映射完成事件、最终 `TextDelta`、`ReasoningCompleted` 或 `FileChangeUpdated` | fileChange 与 reasoning/command 合并进同一工具调用 block；外层汇总“编辑了文件”，展开后由“已编辑的文件”二级 disclosure 控制内嵌逐文件 diff 卡片 | 部分接入 | 其他 item.type／非文本输入继续 fail-fast；fileChange live/history/patch/diff 与双层 disclosure 回归覆盖 |
...
| `item/started` | 服务端通知 | 默认 | 校验 `threadId/turnId`；要求对象 `item` 及字符串 `type/id`；当前严格承接文本 `userMessage`、`agentMessage`、`commandExecution`、`reasoning` 与 `fileChange`；fileChange 严格读取结构化 changes 与状态 | 单 reader → `ManagedTurn`（response 前可缓存）→ `process_turn_message`、各 item parser | 只向 owner turn 映射对应 started/update 事件 | 建立目标 Composer 的活动；fileChange 以稳定 item id 进入当前工具调用 block，并由后续 patch/completed 原位更新二级 disclosure 与内嵌 diff | 部分接入 | 其他 item.type／非文本输入继续 fail-fast；early item 与 turn response 关联、跨 thread 隔离由 manager 回归覆盖 |
