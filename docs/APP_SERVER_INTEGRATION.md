...
当前接入统计：已接入 59、后端已接入 2、部分接入 4、未接入 182。未接入的客户端方法不会发送；未接入的服务端请求按原 id 回复 `-32601` 后 fail-fast；未接入的服务端通知收到即 fail-fast。升级 Codex CLI 时重新生成 experimental schema 并直接核对、更新本表。0.152.1 不存在 `isPinned`，置顶通过服务端返回的 `Pinned` thread section ID 实现。
...
reasoning item 以 `item.id` 独立建模，严格保存 `startedAtMs`／`completedAtMs`，并按 `summaryIndex`／`contentIndex` 承接稀疏数组及增量文本；同一 turn 内的多个 reasoning item 不会合并或串线。`commandExecution.commandActions` 不再只做 schema 校验后丢弃，而是以 `read`／`listFiles`／`search`／`unknown` 的 agent-neutral 结构保留；一个 commandExecution 的每个 action 分别渲染一行，读取行提供文件链接，只有 shell/unknown 行拥有二级详情折叠。`imageView` 的 `id/path` 在实时 started/completed 与历史分页中统一映射为 agent-neutral 图像活动，started/completed 以稳定 id 原位更新，不重复插入；当前 turn 默认展开、历史默认折叠，点击 21 px 标题行展开 80 px 缩略图，再点击缩略图进入全局预览，键盘 disclosure 与全屏预览（背景／关闭／Escape 退出、下载及缩放）均按桌面 ChatGPT CDP 实测行为实现。UI 将相邻 reasoning 与 command item 折叠为一个桌面 ChatGPT 语义的工具调用组：运行态默认展开并以最新 reasoning 短标题作为 21 px 标题行，完成态默认折叠并汇总为“已读取文件”“运行了命令”等动作文案；组内行间距 4 px、滚动区最高 224 px、边缘渐隐 24 px，鼠标及 Enter／Space 共用折叠路径，300 ms `cubic-bezier(.19, 1, .22, 1)` 高度／透明度过渡遵循 reduced-motion；标题箭头另使用桌面端 300 ms `cubic-bezier(.4, 0, .2, 1)` 旋转。没有相邻命令的 reasoning 仍独立渲染，活动时自动展开并跟随最新内容，完成后显示“思考了 {elapsed}”或“完成思考”，正文最高 140 px。
...
| `thread/items/list` | 客户端请求 | 默认 | `threadId`、nullable `turnId`、cursor/limit、`sortDirection=asc`；读取 item 分页 | `CodexAppServerManager::list_thread_items_blocking` → `WorkspaceStore::load_history` 的按需回退 | `Page<ThreadHistoryItemEntry>`，包括 `imageView.id/path` 在内的 item 转为中立历史事件 | `thread/turns/list(itemsView=full)` 仍返回 summary／notLoaded 时，按 turn 逐页补齐 transcript；图像活动恢复 disclosure 与缩略图 | 已接入 | scripted server 覆盖 command/imageView item 与 cursor；fake backend 覆盖非 full turn 触发 item 分页且不重复加载 full turn |
...
| `thread/turns/list` | 客户端请求 | 默认 | `threadId`、cursor/limit、`sortDirection=asc`、`itemsView=full`；读取 turn/item、实际 `itemsView` 与双向 cursor | `CodexAppServerManager::list_thread_turns_blocking` → `WorkspaceStore::load_history` | `Page<ThreadTurn>` → 多轮 `ThreadHistory`，`imageView` 保留稳定 id 与本地路径 | 逐页恢复用户、助手、reasoning、命令、图像查看等 transcript；若响应仍非 full，再按 turn 调用 `thread/items/list`；历史 loading／error／retry | 已接入 | scripted item/视图映射覆盖 imageView；fake backend 多页、items 回退与 Composer 历史＋实时多轮回归 |
...
| `item/completed` | 服务端通知 | 默认 | 要求对象 `item` 及字符串 `type/id`；当前严格承接文本 `userMessage`、`agentMessage`、`commandExecution`、`reasoning`、`fileChange` 与 `imageView`；imageView 严格读取字符串 `path`；fileChange 严格读取 `status` 及每项 `path/diff/kind`，包括 update 的 nullable `move_path`；commandExecution 严格读取并保留 `commandActions` 的 `read`／`listFiles`／`search`／`unknown` 变体，reasoning 另要求整数 `completedAtMs` 及字符串数组 `summary/content` | 单 reader → `ManagedTurn`（response 前按 wire order 缓存）→ `process_turn_message`、各 item parser | 只向 owner turn 映射完成事件、最终 `TextDelta`、`ReasoningCompleted`、`FileChangeUpdated` 或稳定 id 的 `ImageViewed` | imageView 显示 ChatGPT 同构的“已查看 1 张图像” disclosure、80 px 缩略图和全屏预览；fileChange 与 reasoning/command 合并进同一工具调用 block | 部分接入 | 其他 item.type／非文本输入继续 fail-fast；imageView started/completed 去重、字段严格性与 live/history 回归覆盖 |
...
| `item/started` | 服务端通知 | 默认 | 校验 `threadId/turnId`；要求对象 `item` 及字符串 `type/id`；当前严格承接文本 `userMessage`、`agentMessage`、`commandExecution`、`reasoning`、`fileChange` 与 `imageView`；imageView 严格读取字符串 `path`，fileChange 严格读取结构化 changes 与状态 | 单 reader → `ManagedTurn`（response 前可缓存）→ `process_turn_message`、各 item parser | 只向 owner turn 映射对应 started/update 事件，包括 agent-neutral `ImageViewed` | 建立目标 Composer 的活动；imageView 当前 turn 默认展开并在 completed 时按 id 原位更新；fileChange 以稳定 item id 进入当前工具调用 block | 部分接入 | 其他 item.type／非文本输入继续 fail-fast；imageView schema/去重、early item 与 turn response 关联、跨 thread 隔离由回归覆盖 |
