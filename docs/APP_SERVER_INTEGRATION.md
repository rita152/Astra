...
| `item/completed` | 服务端通知 | 默认 | 要求对象 `item` 及字符串 `type/id`；当前严格承接文本 `userMessage`、`agentMessage`、`commandExecution` 与 `reasoning`；commandExecution 严格读取并保留 `commandActions` 的 `read`／`listFiles`／`search`／`unknown` 变体，reasoning 另要求整数 `completedAtMs` 及字符串数组 `summary/content` | 单 reader → `ManagedTurn`（response 前按 wire order 缓存）→ `process_turn_message`、`validate_user_message`、`parse_command_execution`、`parse_reasoning` | 只向 owner turn 映射完成事件、最终 `TextDelta` 或 `ReasoningCompleted`；命令动作保留在 `CommandExecutionAction` | 只完成目标 Composer 的 item；相邻 reasoning/command 形成可折叠工具组并以动作语义生成行文案，reasoning 完成 payload 覆盖流式快照，不结束其他 turn | 部分接入 | 其他 item.type／非文本输入继续 fail-fast；多 thread 与 early-notification 路由由 manager 测试覆盖；`command_execution_preserves_structured_actions_for_activity_rendering`、`reasoning_item_lifecycle_and_all_deltas_map_to_agent_events`、`reasoning_schema_errors_fail_fast` 覆盖命令动作与 reasoning schema 边界 |
...
| `item/started` | 服务端通知 | 默认 | 校验 `threadId/turnId`；要求对象 `item` 及字符串 `type/id`；当前严格承接文本 `userMessage`、`agentMessage`、`commandExecution` 与 `reasoning`；commandExecution 严格读取并保留 `commandActions`，reasoning 另要求整数 `startedAtMs` 及字符串数组 `summary/content` | 单 reader → `ManagedTurn`（response 前可缓存）→ `process_turn_message`、`validate_user_message`、`parse_command_execution`、`parse_reasoning` | 只向 owner turn 映射 `AssistantMessageStarted`／`CommandStarted`／`ReasoningStarted`；命令动作保留在 `CommandExecutionAction` | 建立目标 Composer 的活动；连续 reasoning/command 合并为运行态展开组，最新 reasoning 标题显示在组标题行并自动跟随最新命令 | 部分接入 | 其他 item.type／非文本输入继续 fail-fast；early item 与 turn response 关联、跨 thread 隔离由 manager 回归覆盖；命令动作保留、reasoning 生命周期和 schema 错误测试覆盖 |
