        ...
        for (run_ix, run) in layout.runs.iter().enumerate() {
            ...
            for (glyph_ix, glyph) in run.glyphs.iter().enumerate() {
                ...
                if max_glyph_bounds.intersects(&content_mask.bounds) {
                    ...
                    // Chromium snaps the CSS baseline down before Skia glyph
                    // rasterization. GPUI's nearest-pixel rounding otherwise
                    // moves fractional line heights (e.g. 22.75px) down a row.
                    #[cfg(target_os = "macos")]
                    let paint_origin = point(paint_origin.x, paint_origin.y.floor());
                    if glyph.is_emoji {
                        window.paint_emoji(
                            paint_origin,
                            run.font_id,
