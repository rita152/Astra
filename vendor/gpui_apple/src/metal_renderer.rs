...
fn build_pipeline_state(
    ...
    // The drawable is premultiplied and is composited over native vibrancy.
    // Match RGB's source-over operation for alpha as well: As + Ad * (1 - As).
    // Adding Ad unattenuated makes translucent glyph edges too opaque and dark
    // on a light sidebar even when the glyph bitmap and CSS color are correct.
...
mod compositing_tests {
    use super::*;
    use gpui::{
        MonochromeSprite, PlatformAtlas, Quad, RenderSvgParams, TransformationMatrix, rgba,
    };
    use std::borrow::Cow;

    // Exercise the actual GPU pipelines and read their premultiplied pixels.
    // Opaque-only screenshots cannot detect additive destination alpha.
    #[test]
    fn glyph_coverage_uses_source_over_on_translucent_and_opaque_surfaces() {
        objc::rc::autoreleasepool(|| {
            let mut renderer = MetalRenderer::new_headless(Arc::default());
            ...
            for (background, foreground) in [(255u8, 26u8), (24, 223)] {
                for background_alpha in [0u8, 178, 255] {
                    for coverage in [0u8, 64, 128, 255] {
                        let tile = renderer
                            .sprite_atlas
                            .get_or_insert_with(
                                &RenderSvgParams {
                                    path: format!("coverage-{coverage}").into(),
                        ...
                        scene.insert_primitive(Quad {
                            bounds,
                            content_mask: mask,
                            background: rgba(u32::from_be_bytes([
                                background,
                                background,
                                background,
                                background_alpha,
                        ...
                        scene.insert_primitive(MonochromeSprite {
                            ...
                            bounds,
                            content_mask: mask,
                            color: rgba(u32::from_be_bytes([
                                foreground, foreground, foreground, 217,
                            ]))
                            .into(),
                            tile,
                        ...
                        let source_alpha = coverage as f32 / 255.0 * 217.0 / 255.0;
                        let destination_alpha = background_alpha as f32 / 255.0;
                        let expected_rgb = foreground as f32 * source_alpha
                            + background as f32 * destination_alpha * (1.0 - source_alpha);
                        let expected_alpha =
                        ...
                        for (actual, expected) in pixel.into_iter().zip([
                            ...
                            assert!(
                                ...
                                "background={background}/{background_alpha} coverage={coverage}: {pixel:?}, expected RGB={expected_rgb} alpha={expected_alpha}"
                            );
                        }
                    }
                }
