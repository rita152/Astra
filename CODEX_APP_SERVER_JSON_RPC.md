...
| `item/commandExecution/requestApproval` | 服务端 → 客户端反向请求 | `AgentEvent::CommandApprovalRequested` + `AgentApprovalHandle` | 保存原始 string/int64 request id、完整 params 与每请求的 `availableDecisions`；底部审批卡临时替代 Composer。允许一次回 `accept`；可见的“拒绝”与 ChatGPT App 一致始终回 `decline`，即使当前 app-server 只广告 `cancel`，因为 `cancel` 会中断整个 turn；“允许类似命令”原样回传服务端的 `acceptWithExecpolicyAmendment`。若早于 `turn/start` 响应则先暂存，校验 thread/turn 后才注册和显示 |
...
Prompt 会话的 stdin 由可并发写入的 `CodexTurnSession` 持续持有，当前 `threadId` 与 `turnId` 会保留到终态。同一个 session 还维护线程安全的多请求 command approval registry；decision 在写 stdin 前会再次校验该请求的 `availableDecisions` 并原子标记已回复，因此重复点击、resolved 后点击和跨请求 decision 都不会产生第二次写入。唯一兼容例外是请求广告 `cancel` 时，可见的“拒绝”按 ChatGPT App 行为回 `decline`，避免把拒绝单条命令升级为中断整个 turn。点击停止后 Composer 只进入 `Stopping`，不会截断事件流或伪造本地完成；只有收到匹配 turn 的 `turn/completed` 且状态为 `interrupted` 后才转为 `Stopped`。若任务先自然完成，则保留 `Complete`；若中断写入或连接失败，则进入 `Failed`。`thread/resume` 的 JSON-RPC error、缺少 `result.thread.id` 或返回不一致 id 都会 fail closed：不发送 `turn/start`，更不会静默回退到 `thread/start`；原 Composer thread id 保持不变，stdin 和 app-server 子进程仍按协议失败路径关闭、kill 并 wait。若恢复 active goal 时 app-server 在 `turn/start` 响应前自动发出旧 turn 消息，适配器不会提前污染当前事件流或审批 registry：整批暂存的 started/error/model、item、completion、command approval 与 resolved 只有在相关 `threadId`、`turnId` 与新响应一致时才按原顺序重放；响应失败或任一 id 不一致时直接失败并丢弃整批事件。活动阶段的同类消息也逐条校验当前 thread/turn。控制句柄丢弃、协议异常和正常终态都走同一幂等回收路径，避免重复停止与退出竞态留下子进程。

真实 CLI smoke test 使用 Codex CLI 0.151.0 与 ChatGPT App 内置的 0.151.0-alpha.7.2 分别申请在 sandbox 外执行只读 `pwd`。两者都只广告 `accept`、`acceptWithExecpolicyAmendment` 与 `cancel`（没有 `decline`），但 ChatGPT App 前端的“拒绝”固定回 `{"decision":"decline"}`。对同一请求回 `cancel` 时，command item 以 `declined` 结束且 turn 以 `interrupted` 结束；回未广告的 `decline` 时，两版 app-server 都让 command item 以 `declined` 结束、继续生成“未执行命令”的 agent message，最后 turn 以 `completed` 结束。因此 GPUI 的可见“拒绝”也固定发送 `decline`，`cancel` 仅保留为取消整个回合的协议值。
